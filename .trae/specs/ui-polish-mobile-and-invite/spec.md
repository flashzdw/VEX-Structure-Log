...
### Requirement: 移动端汉堡按钮尺寸 + 位置
系统 SHALL 在移动端（< 640px）下让展开 / 关闭菜单的按钮放在顶栏右上角，且比桌面端略小。
- 按钮位置：顶栏右侧（最右端），始终在同一坐标
- 按钮尺寸：比桌面端 Tab 链接略小（`p-1.5` 圆角胶囊 + `w-4 h-4` 图标）
- 展开时按钮图标从 `Menu` 切换为 `X`，仍在同一位置
- 关闭浮层时按钮图标恢复为 `Menu`
- 桌面端（≥ 640px）下按钮不显示

#### Scenario: 移动端点击展开
- **WHEN** 屏幕宽度 < 640px 且用户点击右上角 `Menu` 按钮
- **THEN** 同一按钮变为 `X` 图标，浮层在按钮下方展开

#### Scenario: 移动端点击关闭
- **WHEN** 浮层处于打开状态且用户点击右上角 `X` 按钮
- **THEN** 同一按钮恢复为 `Menu` 图标，浮层关闭
